...
<div class="mb-6 flex justify-between items-center">
    <a href="{{ url("/my-courses") }}" class="text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4
...
<div class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
    ...
    <div class="bg-white dark:bg-gray-800 shadow-md p-4 rounded-lg mb-4 mt-4">
        ...
        <p class="text-md">{{ Str::limit($content->materi, 100) }}</p>

        @if ($content->progress > 0)
        <p class="text-green-500 mt-2 block">Sudah Dipelajari (Progress: {{ $content->progress }}%)</p>
        @else
        <a href="{{ route('student.contents.show', ['content' => $content->id]) }}"
            class="text-blue-500 hover:underline mt-2 block">Mulai Belajar</a>
        @endif
    </div>
    @endforeach
</div>
