...
@section('content')
<form method="POST" action="{{ route('teacher.contents.destroy', ['content' => $content->id]) }}" class="mb-6 flex justify-between
    items-center">
    @csrf
    @method('DELETE')
    <a href="{{ url()->previous() }}"
...
<form method="POST" action="{{ route('teacher.contents.update', ['content' => $content->id]) }}">
    @csrf
    @method('PUT')
