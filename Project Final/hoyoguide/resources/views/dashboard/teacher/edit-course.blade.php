...
@section('content')
<form method="POST" action="{{ url("/course/{$selectedCourse->id}") }}" class="mb-6 flex justify-between items-center">
    @csrf
    @method('DELETE')
    <a href="/course"
...
<form method="POST" action="{{ url("/course/{$selectedCourse->id}") }}">
    @csrf
    @method('PUT')
    ...
    <div class="mb-6 flex justify-between items-center gap-3">
        <div class="w-10/12">
            <label for="content" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tanggal Selesai</label>
